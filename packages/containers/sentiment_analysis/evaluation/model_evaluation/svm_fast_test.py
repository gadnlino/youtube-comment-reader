#!/usr/bin/env python3
"""
Fast SVM + TF-IDF Sentiment Analysis

This script performs sentiment analysis using Support Vector Machine (SVM) 
with TF-IDF vectorization on a smaller dataset for faster results.
"""
...
def create_tfidf_vectorizer(max_features=5000, ngram_range=(1, 1)):
    """Create TF-IDF vectorizer with smaller feature set"""
...
def train_svm_model(X_train, y_train, C=1.0, kernel='linear'):
    """Train SVM model with linear kernel for speed"""
...
def display_results(results, processing_time, sample_size):
    ...
    print("\n" + "="*80)
    print("FAST SVM + TF-IDF SENTIMENT ANALYSIS RESULTS")
    print("="*80)
...
def save_results(results, processing_time, sample_size, model, vectorizer, label_encoder):
    ...
    with open('svm_fast_results.txt', 'w') as f:
        f.write("Fast SVM + TF-IDF Sentiment Analysis Results\n")
        f.write("="*60 + "\n\n")
    ...
    with open('svm_fast_model.pkl', 'wb') as f:
        pickle.dump(model_data, f)
    
    print("Results saved to svm_fast_results.txt")
    print("Model saved to svm_fast_model.pkl")


def main():
    """Main function"""
    print("Fast SVM + TF-IDF Sentiment Analysis")
    print("="*60)
    
    # Configuration - much smaller for speed
    sample_size = 10000  # Use 10k samples for fast training
    test_size = 0.2
    random_state = 42
    ...
    # Create TF-IDF features with smaller feature set
    print(f"\nCreating TF-IDF features...")
    vectorizer = create_tfidf_vectorizer(max_features=5000, ngram_range=(1, 1))
    ...
    svm_model = train_svm_model(X_train_tfidf, y_train, kernel='linear')
