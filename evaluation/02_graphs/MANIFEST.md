...
Caminhos canónicos dos **sete activos** embebidos no documento Word da monografia (Tabela 1–4, Figura 23–26). Figuras de rascunho em markdown (`multiple_sets_*`, gráficos de análise de idioma, gráficos PT da API) **não** estão nesta lista e foram arquivadas em `evaluation/06_archived/pruned_figures/2026-06/`.

Inventário legível por máquina: [`inventory.json`](inventory.json).

## Lista de activos (keep-list)

| Docx | Tipo | Caminho canónico | Gerador | Dados de suporte |
|------|------|------------------|---------|------------------|
...
| **Figura 24** | Barras selecção vs Kaggle | [`figures/figura-24_tfidf_lr_selection_vs_kaggle_amitzala.png`](figures/figura-24_tfidf_lr_selection_vs_kaggle_amitzala.png) | [`../scripts/01_model_evaluation/plot_tfidf_lr_selection_vs_kaggle_amitzala.py`](../scripts/01_model_evaluation/plot_tfidf_lr_selection_vs_kaggle_amitzala.py) | CSV Kaggle AmitZala + métricas de selecção (ver docstring do script) |
...
### Linhagem das fontes (cópias canónicas)
...
# Tabela 4 (PNG canónico em tables/tabela-4_e2e_test_results_table.png)
...
Após regenerar, copie os novos PNGs para `figures/` ou `tables/` com os nomes canónicos acima (excepto a Tabela 4 com `--thesis`, que já grava no caminho canónico).

## Excluídos da lista de activos (arquivados)
...
- `generate_e2e_test_table.py` em execução de fumo produz o PNG da Tabela 4 (dados de teste embutidos).
- Os outros geradores exigem entradas externas documentadas acima; os PNGs canónicos estão preservados em `figures/` e `tables/`.
