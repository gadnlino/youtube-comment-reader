...
def generate_language_comparison_bar_chart(data, timestamp):
    ...
    plt.tight_layout()
    filename = f'comparacao_viés_neutral_idiomas_pt_{timestamp}.png'
...
def generate_individual_video_chart(data, timestamp):
    ...
    plt.tight_layout()
    filename = f'taxa_neutral_videos_individuais_pt_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✓ Gerado: {filename}")
    plt.close()
...
def generate_sentiment_distribution_heatmap(data, timestamp):
    ...
    plt.tight_layout()
    filename = f'heatmap_distribuicao_sentimentos_pt_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✓ Gerado: {filename}")
    plt.close()
...
def generate_language_bias_statistical_plot(data, timestamp):
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='D', color='w', markerfacecolor='green', markersize=8, label='Média'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=8, 
               alpha=0.6, markeredgecolor='black', label='Vídeos Individuais')
    ]
    ax.legend(handles=legend_elements, loc='upper right', fontsize=10)
    
    plt.tight_layout()
    filename = f'boxplot_viés_linguistico_pt_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✓ Gerado: {filename}")
    plt.close()
...
def generate_stacked_bar_comparison(data, timestamp):
    ...
    plt.tight_layout()
    filename = f'comparacao_sentimentos_multilíngue_pt_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✓ Gerado: {filename}")
    plt.close()
...
def main():
    ...
    # Carregar resultados mais recentes
    import glob
